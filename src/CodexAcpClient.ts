...
import type { GetAccountRateLimitsResponse } from "./app-server/v2";
 ...
 */
export type CreateUrlElicitationRequest = Extract<
 ...
 */
export type UrlElicitationRequest = Omit<
...
export class CodexAcpClient {
  ...
  private pendingLoginCompleted: Promise<AccountLoginCompletedNotification> | null =
    null;
  private pendingAccountUpdated: Promise<AccountUpdatedNotification> | null =
    null;
  ...
  private readonly accountLimitsHandlers = new Set<
    (snapshot: AccountLimitsSnapshot) => void
  >();
  ...
  onAccountLimitsUpdated(
    handler: (snapshot: AccountLimitsSnapshot) => void,
  ): void {
  ...
  async authenticate(
    ...
    switch (authRequest.methodId) {
      ...
      case "chat-gpt-device-code":
        return await this.authenticateWithChatGptDeviceCode(
  ...
  private async authenticateWithApiKey(
    authRequest: ApiKeyAuthRequest,
  ): Promise<Boolean> {
    const apiKey =
      authRequest._meta?.["api-key"]?.apiKey ?? this.readApiKeyFromEnv();
  ...
  private async authenticateWithChatGpt(): Promise<Boolean> {
    const accountResponse = await this.codexClient.accountRead({
      refreshToken: true,
    });
    ...
    const loginCompletedPromise = this.awaitNextLoginCompleted();
    const loginResponse = await this.codexClient.accountLogin({
      type: "chatgpt",
    });
  ...
  private async authenticateWithChatGptDeviceCode(
    urlElicitationRequester?: UrlElicitationRequester,
  ): Promise<Boolean> {
    const accountResponse = await this.codexClient.accountRead({
      refreshToken: true,
    ...
    if (!urlElicitationRequester) {
      throw RequestError.invalidRequest(
        undefined,
        "Device code authentication requires URL elicitation support",
      );
    }
    const loginCompletedPromise = this.awaitNextLoginCompleted();
    const loginResponse = await this.codexClient.accountLogin({
    ...
    const elicitationResponsePromise = Promise.resolve(
      urlElicitationRequester.elicitUrl({
    ...
    if (!acp.CreateElicitationResponse.isAccept(first.response)) {
      await this.codexClient.accountLoginCancel({
  ...
  async getCurrentModelProvider(): Promise<string | null> {
    ...
    const settingsModelProvider = await this.codexClient.configRead({
      includeLayers: false,
    });
  ...
  async authRequired(): Promise<Boolean> {
    ...
    const response = await this.codexClient.accountRead({
      refreshToken: false,
    });
  ...
  private getNativeProviderConfig(): acp.ProviderCurrentConfig {
    const configuredProviderId =
      this.modelProvider ??
      (typeof this.config["model_provider"] === "string"
        ? this.config["model_provider"]
        : null);
    ...
    if (
      ...
      if (
        ...
        !Array.isArray(configuredProvider)
      ) {
        const baseUrl = (configuredProvider as Record<string, unknown>)[
          "base_url"
        ];
  ...
  isClientConfiguredProvider(providerId: string | null): boolean {
    return (
      ...
      this.gatewayConfigSource === "acpProviders"
    );
  }
  ...
  async getAgentConfiguredModelProvider(): Promise<string | null> {
    ...
    const agentProvider = this.isClientConfiguredProvider(provider)
      ? this.modelProvider
      : provider;
  ...
  async resumeSession(
    ...
    const currentModelId = this.createModelId(
      codexModels,
      response.model,
  ...
  async loadSession(
    ...
    onSubscribed?: () => void,
  ): Promise<SessionMetadataWithThread> {
    const additionalDirectories = readAdditionalDirectories(
      request.cwd,
      request.additionalDirectories,
      request._meta,
  ...
  async readSessionThread(sessionId: string): Promise<Thread> {
    return (
      await this.codexClient.threadRead({
        threadId: sessionId,
        includeTurns: true,
      })
    ).thread;
  ...
  async getGoal(sessionId: string): Promise<ThreadGoal | null> {
    const response = await this.codexClient.threadGoalGet({
      threadId: sessionId,
    });
  ...
  async awaitMcpServerStartup(
    ...
    return await this.codexClient.awaitMcpServerStartup(
      serverNames,
      afterVersion,
    );
  }
  ...
  private async createSessionConfig(
    ...
    return {
      ...
      mcp_servers: Object.fromEntries(
        serversToConfigure.map((mcp) => [
          mcp.name,
          this.createMcpSeverConfig(mcp.server),
        ]),
      ),
    };
  }

  private async getConfigMcpServerNames(
    projectPath: string,
  ): Promise<Set<string>> {
    const response = await this.codexClient.configRead({
      includeLayers: true,
      cwd: projectPath,
    });
    ...
    const layerMcpServers = configLayers.map((layer) => {
      return isJsonObject(layer.config)
        ? layer.config["mcp_servers"]
        : undefined;
    });
    ...
    return new Set(
      configuredMcpServers.flatMap((server) => Object.keys(server)),
    );
  }
  ...
  private createMcpSeverConfig(mcpServer: McpServer): JsonObject {
    if ("type" in mcpServer) {
      switch (mcpServer.type) {
        case "acp":
          throw RequestError.invalidRequest(
            "Codex doesn't support MCP ACP transport protocol",
          );
        case "sse":
          throw RequestError.invalidRequest(
            "Codex doesn't support MCP SSE transport protocol",
          );
        case "http":
          return {
            url: mcpServer.url,
            http_headers: Object.fromEntries(
              mcpServer.headers.map((h) => [h.name, h.value]),
            ),
          };
      }
    }
    return {
      ...
      env: Object.fromEntries(
        mcpServer.env.map((env) => [env.name, env.value]),
      ),
    };
  }
  ...
  createModelId(
    ...
    if (!defaultModel) {
      throw new Error(
        `Model selection failed: No model found for ID "${modelId}" and no default model is defined.`,
      );
    }
  ...
  async subscribeToSessionEvents(
    ...
    this.subagents.subscribe({
      ...
      enqueueInteraction: (event) => {
        ...
        this.enqueueSessionNotification(sessionId, () =>
          observeInteraction(event),
        );
      },
      approvalHandler,
      elicitationHandler,
      waitForRootNotifications: () =>
        this.waitForSessionNotifications(sessionId),
      waitForChildSession,
    });
  }
  ...
  private enqueueSessionNotification(
    sessionId: string,
  ...
  async runAgentFileChangeReport(params: {
    ...
    if (params.signal?.aborted) {
      return createUnavailableAgentFileChangeReport(
        params.requestId,
        "cancelled",
      );
    }
    ...
    try {
      ...
      void forkPromise.then(
        (fork) => {
          if (lateStopReason !== null && forkThreadId === null) {
            void this.unsubscribeAgentFileChangeReportThread(
              fork.thread.id,
              budget,
            );
          }
      ...
      const turnPromise = this.codexClient.runTurn(
        ...
        (turnId) => {
          auditTurnId = turnId;
          if (lateStopReason !== null && forkThreadId !== null) {
            void this.interruptAgentFileChangeReport(
              forkThreadId,
              turnId,
              lateStopReason,
              budget,
  ...
  private async interruptAgentFileChangeReport(
    ...
    try {
      ...
      logger.error(
        `Failed to interrupt ${reason} agent file-change report`,
        error,
      );
    } finally {
  ...
  private async unsubscribeAgentFileChangeReportThread(
    ...
    try {
      ...
      logger.error(
        "Failed to unsubscribe the agent file-change report thread",
        error,
      );
    }
  }
  ...
  private getCollaborationMode(sessionId: string): ModeKind {
    return (
      this.codexClient.getThreadSettings(sessionId)?.collaborationMode.mode ??
      "default"
    );
  }
  ...
  private async awaitSingleNotification<T>(
    ...
    return await new Promise((resolve) => {
      ...
      disposable = this.codexClient.connection.onNotification(
        method,
        (event: T) => {
          disposable?.dispose();
  ...
  async awaitMcpServerOauthLoginCompleted(
    ...
    return await this.codexClient.awaitMcpServerOauthLoginCompleted(
      name,
      threadId,
    );
  }
  ...
  async listSessions(
    ...
    if (requestedCwd) {
      ...
      if (filtered.length > 0 || isAbsolutePathLike(requestedCwd)) {
        ...
        logger.log("Ignoring non-absolute cwd filter for session/list", {
          cwd: requestedCwd,
        });
      }
    }
  ...
  async fetchAvailableModels(): Promise<Model[]> {
    ...
    do {
      const response = await this.codexClient.listModels({
        cursor,
        limit: null,
      });
  ...
  private async runSessionListDiagnostics(): Promise<Record<string, unknown>> {
    const [allProviders, archivedAllProviders, customGateway] =
      await Promise.all([
        this.codexClient.threadList({}),
        this.codexClient.threadList({ archived: true }),
        this.codexClient.threadList({
          modelProviders: [CUSTOM_GATEWAY_PROVIDER_ID],
        }),
      ]);
...
class AgentFileChangeReportBudget {
  ...
  async wait<T>(operation: Promise<T>): Promise<T> {
    ...
    return await new Promise<T>((resolve, reject) => {
      ...
      const onAbort = (): void =>
        finish(() => reject(new AgentFileChangeReportBudgetError("cancelled")));
...
function buildPromptItems(prompt: acp.ContentBlock[]): UserInput[] {
  return prompt
    .map((block): UserInput | null => {
      switch (block.type) {
        case "text":
          return { type: "text", text: block.text, text_elements: [] };
        case "image": {
          const url = isSupportedImageUrl(block.uri)
            ? block.uri
            : imageDataUrl(block);
        ...
        case "resource_link":
          return {
            type: "text",
            text: formatUriAsLink(block.name, block.uri),
            text_elements: [],
          };
        case "resource": {
...
function isSupportedImageUrl(uri: string | null | undefined): uri is string {
  ...
  try {
    ...
    return (
      protocol === "http:" || protocol === "https:" || protocol === "data:"
    );
  } catch {
    return false;
  }
...
function readMetaAdditionalRoots(
...
function mergeSandboxWorkspaceWriteRoots(
  config: JsonObject,
  roots: string[],
): JsonObject {
  if (roots.length === 0) {
  ...
  const existingSandboxConfig = isJsonObject(config["sandbox_workspace_write"])
    ...
    : {};
  const existingWritableRoots = Array.isArray(
    existingSandboxConfig["writable_roots"],
  )
    ? existingSandboxConfig["writable_roots"].filter(
...
function gatewayApiTypeFromConfig(
  gatewayConfig: GatewayConfig,
): acp.LlmProtocol {
  const wireApi = gatewayConfig.config.wire_api;
  const match = Object.entries(SUPPORTED_GATEWAY_PROTOCOLS).find(
    ([, wire]) => wire === wireApi,
  );
...
function mergeGatewayConfig(
  ...
  if (gatewayConfig !== null) {
    ...
    if (
      ...
      typeof newConfig["model_providers"] !== "object"
    ) {
      newConfig["model_providers"] = {};
    } else {
      newConfig["model_providers"] = {
        ...(newConfig["model_providers"] as JsonObject),
      };
    }

    newConfig["model_providers"][gatewayConfig.modelProvider] =
